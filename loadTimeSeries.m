function [ im ] = loadTimeSeries( goodFrames, goodchannels, dataDir )
...
im = zeros(size(imtemp,1),size(imtemp,2),numel(goodFrames),numel(goodchannels));

if goodchannels>0

for j = 1:numel(goodinds)
    im(:,:,frames(goodinds(j))-min(goodFrames)+1,channels(goodinds(j))-min(goodchannels)+1) = imread(tifstruct(goodinds(j)).name);
...
for j = 1:numel(goodinds)
    im(:,:,frames(goodinds(j))-min(goodFrames)+1) = imread(tifstruct(goodinds(j)).name);
%     channelsOut(j) = channels(goodinds(j)); %For Debugging
%     zOut(j) = z(goodinds(j));
%     framesOut(j) = frames(goodinds(j));
end
